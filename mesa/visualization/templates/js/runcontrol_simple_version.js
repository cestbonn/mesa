...
const initGUI = function (model_params) {
  ...
  const addModifyChoice2 = function (param, obj) {
    function createDropdowns(data, parent) {
        ...
        dropdowns.id = createDropdown('id', 'Purchase ID\n購入ID', Object.keys(data));
        dropdowns.supplier = createDropdown('supplier', 'Supplier\nサプライヤー', firstIdData.supplier.options, firstIdData.supplier.value);
        dropdowns.transition = createDropdown('transition', 'Transportation Method\n輸送方法', firstIdData.transition.options, firstIdData.transition.value);
